...
def blink_freq(orig):
    ...
    if blink_freq_stat==0:
        ...
        cv.createTrackbar('Low H', 'LED Blinking Frequency' , 17, 179, nothing)
        cv.createTrackbar('High H', 'LED Blinking Frequency' , 25, 179, nothing)
        cv.createTrackbar('Low S', 'LED Blinking Frequency' , 0, 255, nothing)
        cv.createTrackbar('High S', 'LED Blinking Frequency' , 7, 255, nothing)
        cv.createTrackbar('Low V', 'LED Blinking Frequency' , 235, 255, nothing)
    ...
    height,width,channels=orig.shape
    orig=roi(orig,'LED blinking ROI Selector',193,height,363,width) #357,480,5,205
    height,width,channels=orig.shape
    orig=cv.GaussianBlur(orig,(5,5),0)
    hsv = cv.cvtColor(orig, cv.COLOR_BGR2HSV)
    ...
    if freq_being_found==False:
        ...
        if M['m00']!=0:
            ...
            if cY>0.3*height: #and timer_started==False:
                #startt=time.time()
                freq_being_found=True
                freq_count+=1
               # print("the ratio of led height : ", cY/height)
            #timer_started=True
...
def path_detection(orig):
    ...
    if path_detection_stat==0:
        ...
        cv.createTrackbar('Threshold', 'Path Binary' , 13, 255, nothing)
